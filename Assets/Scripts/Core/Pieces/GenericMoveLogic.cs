...
namespace Antichess.Core.Pieces
{
    public static class GenericMoveLogic
    {
        ...
        private static void AddMovesInDir(Position pos, Position increments,
            Board boardRef, LegalMoves legalMoves)
        {
            ...
            while (to.X < ObjectLoader.BoardSize && to.Y < ObjectLoader.BoardSize)
            {
                ...
                {
                    if (legalMoves.CanTake == false) legalMoves.Add(new Move(pos, to), false);
                }

                else if (boardRef.PieceAt(to).IsWhite != boardRef.PieceAt(pos).IsWhite)
                {
                    legalMoves.Add(new Move(pos, to), true);
                    break;
                }
        ...
        public static void AddLegalMovesInDirections(Position pos,
            IEnumerable<Position> directions, Board boardRef, LegalMoves legalMoves)
        {
