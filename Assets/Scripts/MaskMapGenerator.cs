...
public class MaskMaker : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Space(10f);
        AmbientOcclusion =
            (Texture2D) EditorGUILayout.ObjectField("Ambient Occlusion (G)", AmbientOcclusion, typeof(Texture2D),
                false);
        if (!AmbientOcclusion)
        {
        ...
        if (useRough = EditorGUILayout.Toggle("Input Is Roughness Map", useRough))
            Smooth_Rough =
                (Texture2D) EditorGUILayout.ObjectField("Rough Map (A)", Smooth_Rough, typeof(Texture2D), false);
        else
            Smooth_Rough =
                (Texture2D) EditorGUILayout.ObjectField("Smoothness Map (A)", Smooth_Rough, typeof(Texture2D), false);
        if (!Smooth_Rough)
        {
    ...
    {
        var renderTex = RenderTexture.GetTemporary(source.width, source.height, 0, RenderTextureFormat.ARGBFloat,
